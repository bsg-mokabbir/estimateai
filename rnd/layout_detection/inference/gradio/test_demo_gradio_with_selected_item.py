...
# ==== Prediction Function ====
def predict_image(input_image_path, selected_classes):
    ...
    try:
        ...
        filtered_detections = [d for d in detections if d['class_name'] in selected_classes]

        # Draw only selected detections
        result_img_bgr = model.draw_detections(img_bgr, filtered_detections)
        result_img_rgb = cv2.cvtColor(result_img_bgr, cv2.COLOR_BGR2RGB)
        result_pil = Image.fromarray(result_img_rgb)

        # Count detections (for both all + selected)
        class_counts = {}
        ...
        # Find matched GT
        matched_entry = next((entry for entry in report_data if entry['main name'] == filename), None)

        for det_class in sorted(class_mapping.keys()):
            if det_class not in selected_classes:
                continue
            pred_count = class_counts.get(det_class, 0)
            gt_count = matched_entry.get(class_mapping[det_class], "NA") if matched_entry else "NA"
            if gt_count == 0:
                gt_count = "NA"
            count_text += f"• {det_class}: {pred_count}/{gt_count}\n"

        # Confidence for selected classes only
        if filtered_detections:
            confs = [d['confidence'] for d in filtered_detections]
            count_text += (
                f"\n📈 Confidence:\n"
                f"• Avg: {np.mean(confs):.3f} | "
                f"Max: {np.max(confs):.3f} | "
                f"Min: {np.min(confs):.3f}\n"
            )
        else:
            count_text += "\n❌ No objects detected for selected classes"
...
gr.Interface(
    ...
    inputs=[
        ...
        gr.CheckboxGroup(choices=all_classes, value=all_classes, label="Select Which Components to be Counted")
    ],
    outputs=[
