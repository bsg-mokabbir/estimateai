...
ANNOTATION_FILE = 'data_agumentation\drawn_images\legend_annotations_resize.json'
...
def process_image(image_path, output_path, vgg_annotations):
    ...
    for legend_name, legend_img in legends:
        rotated_instances = random.sample(range(5), 2)  
        for i in range(5):
            tries = 0
            while tries < 100:
                img_to_place = legend_img.rotate(90, expand=True) if i in rotated_instances else legend_img  
                ...
                if all(not is_overlapping(new_box, b) for b in placed_boxes):
                    ...
                    regions.append({
                        ...
                        'region_attributes': {
                            'Layout_detect': legend_name.split(".")[0],
                            'Text': ''
                        }
                    })
                    break
                ...
                print(f"Warning: Could not place {legend_name} instance {i+1} without overlap in {os.path.basename(image_path)}.")
    ...
    for fname in os.listdir(IMAGES_DIR):
        if fname.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            try:
                ...
                process_image(temp_path, output_path, vgg_annotations)
